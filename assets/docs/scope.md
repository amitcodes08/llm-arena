...
- [x] Decide the approach — AI SDK with `@openrouter/ai-sdk-provider` for model calls; three independent `fetch` streams per prompt (one per model) so a single failure never takes down the others.
- [x] Wire it — OpenRouter provider (`app/arena/lib/openrouter.ts`), Prisma 7 with PrismaPg adapter (`app/arena/lib/db.ts`), Clerk via `proxy.ts` + `<ClerkProvider>`, Arcjet shared client (`app/arena/lib/arcjet.ts`), PostHog client + server providers with Clerk user identification (`app/arena/providers/posthog-provider.tsx`, `app/arena/lib/posthog-server.ts`). Fail-fast env validation in `app/env.ts`. Typecheck, lint, and build all pass.
