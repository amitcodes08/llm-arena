...
export const POST = async (request: Request): Promise<Response> => {
  ...
  if (!catalog) {
    return Response.json(
      {
        error:
          "We can't reach the model list right now. Try again in a moment.",
      },
      { status: 503 }
    );
  }
  ...
  if (
    !catalog.some((model: { id: string }) => model.id === parsed.data.modelId)
  ) {
    return Response.json(
  ...
  const target = await database().modelResponse.findFirst({
    where: {
  ...
  if (!appUserId || target.turn.thread.userId !== appUserId) {
    return Response.json(
      { error: "That turn isn't yours to answer." },
      { status: 403 }
    );
  }
